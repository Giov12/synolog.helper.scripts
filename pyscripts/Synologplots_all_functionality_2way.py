...
def get_arguments():
    ...
    return spA, spB, synolog_dir, color_scheme
...
def get_scaffold_lengths(synolog_dir, spA, spB):
    ...
    return spA_dict, spB_dict, spA_genome_length, spB_genome_length
...
def chromB_order(sorted_spA_dict, sorted_spB_dict, ortho_rows):
...
def create_ticks(genome_size):
    ...
    if marker == "K":
        ...
        tick_ct = math.floor(genome_size / 5e9)
        multiplier = 5
...
def plot_scaffolds(
    ...
    # values that will be updated through the process
    current_starting_x_posA = 100
    current_starting_x_posB = 100
    ...
    spA_starting_positions = establish_starting_positions(
        spA_dict, spA_genome_length, image_width, current_starting_x_posA
    )

    spB_starting_positions = establish_starting_positions(
        spB_dict, spB_genome_length, image_width, current_starting_x_posB
    )
...
def main():
    ...
    # get args
    spA, spB, synolog_dir, color_scheme = get_arguments()

    # parse the membership file
    ortho_rows = get_ortho_rows(spA, spB, synolog_dir)

    # parse the chromosomes file
    spA_dict, spB_dict, spA_genome_length, spB_genome_length = get_scaffold_lengths(
        synolog_dir, spA, spB
    )

    # reorder chroms for spB
    spB_dict = chromB_order(spA_dict, spB_dict, ortho_rows)
